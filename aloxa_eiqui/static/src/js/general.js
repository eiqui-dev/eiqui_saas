...
odoo.define('aloxa_eiqui.general', function (require) {
	'use strict';
	...
	if (typeof AloxaEiquiGeneral === 'undefined') {
		var AloxaEiquiGeneral = {
			loadTimeout: false,
			
			////////////////////////
			onLoad: function() {
				$(function(){AloxaEiquiGeneral.onPageLoad();});
			},
			...
			loadEiquiAjax: function(url, params, callback)
			{
				...
				$.post(url, params, function(data){
					clearTimeout(AloxaEiquiGeneral.loadTimeout);
					var $this = $('#eiqui-ajax');
					$this.html(data);
	...
	if (typeof AloxaEiquiPlan === 'undefined') {
		var AloxaEiquiPlan = {
			onPageLoad: function() {
				var $eiqui = $('#eiqui-ajax');
			...
			loadPanelSection: function(project_id, section, payload) {
				...
				AloxaEiquiGeneral.loadEiquiAjax(url, {}, function(data){
					// Initialize Tooltips
					//$(function () {
	...
	if (typeof AloxaEiquiPlanList === 'undefined') {
		var AloxaEiquiPlanList = {
			...
			onPageLoad: function() {
				$('#search_plans').on('submit', function(ev){
					...
					AloxaEiquiGeneral.loadEiquiAjax('/panel/search', {search:search}, function(data){
						$(".disabled").click(function(event) {
							event.preventDefault();
							return false;
						});
